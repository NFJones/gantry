    ...
    for entry in manifest.entries {
        ...
        assert!(
            review
                .evidence
                .iter()
                .any(|evidence| evidence == EVIDENCE_ID),
            "{}:{}",
            entry.requirement,
            entry.clause
        );
...
fn lexical_requirement_vectors_cover_reviewed_clauses() {
    let source = concat!(
        "\u{feff}/* outer /* inner */ */ action agent agents as assert attempt Bool break ",
        "continue crate Decision decide default detach discard effects else enum Err false Float fn ",
        "fork for idempotent if impl in inline Int join joinall let limit List loop Map match mod ",
        "mut new Never non_idempotent None null Ok OperationError Option panic prompt pure ",
        "Range read_only Result ",
        "return retry_limit self Self session Set Some spawn String struct super trait true Tuple ",
