...
fn generated_conformance_manifest_and_corpus_are_exact_and_complete() {
    ...
    assert_eq!(corpus.entries.len(), 129);
    assert_eq!(
        corpus
            .entries
            .iter()
    ...
    assert_eq!(manifest.corpus.evidence_count, 129);
    assert_eq!(manifest.corpus.mapping_count, 2_084);
    ...
    assert_eq!(manifest.manifests.len(), 42);
    assert!(
        manifest
            .manifests
            .windows(2)
