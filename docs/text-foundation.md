...
Every clause of this section is pure and target-independent: no clause consults a host locale, host encoding, host regular-expression behavior, host math library, ambient time, or global mutable state. Every input-dependent kernel of the section has an exact declared bound or budget: the builder's octet bound of `GNT-41.4-canonical-text-builders`, the pattern bounds and match step budget of `GNT-41.8-bounded-text-matching`, and the exact admission refusals of `GNT-41.1-canonical-text-values` and `GNT-41.9-canonical-text-conversions`. The section publishes no cancellation safe point, no memory or time limit beyond those declared bounds and budgets, no resource accounting or quota, no suspension, no durable continuation, and no codec or serialization contract; those remain the runtime's contracts under the owners named above, and this record is the retry condition for the gated rows.
