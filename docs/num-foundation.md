...
`GNT-40.4-integer-bit-operations` admits exactly three binary bit operations — `and`, `or`, and `xor` — and exactly three unary bit operations — `not`, `leading-zeros`, and `trailing-zeros`. Each operation is exact over the two's-complement representation of its operands: a combination or complement publishes exactly one canonical `Int` value when that exact result lies inside the canonical domain and is refused under `integer-overflow` when it does not, so no result wraps, saturates, or is masked into range; the counts published by `leading-zeros` and `trailing-zeros` are canonical `Int` values of at most the representation width of 64 and are total over the domain. `BinaryBitOperation::apply` and `UnaryBitOperation::apply` publish these operations in the model. No shift, rotation, population count, bit field, float algorithm, conversion, parsing, formatting, work limit, cancellation safe point, quota, schema, recovery, durability, boundary encoding, lowering, machine representation, or family behavior is published with them, and no performance, storage layout, or physical-representation claim is made.
