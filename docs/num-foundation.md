...
## Integer bit operations (`GNT-40.4`)

`GNT-40.4-integer-bit-operations` admits exactly three binary bit operations — `and`, `or`, and `xor` — and exactly three unary bit operations — `not`, `leading-zeros`, and `trailing-zeros`. Each operation is exact over the two's-complement representation of its operands: a combination or complement publishes exactly one canonical `Int` value when that exact result lies inside the canonical domain and is refused under `integer-overflow` when it does not, so no result wraps, saturates, or is masked into range; the counts published by `leading-zeros` and `trailing-zeros` are canonical `Int` values of at most the representation width of 64 that `GNT-5.15-canonical-scalar-keys` declares, and are total over the domain. `BinaryBitOperation::apply` and `UnaryBitOperation::apply` publish these operations in the model. No shift, rotation, population count, bit field, float algorithm, conversion, parsing, formatting, work limit, cancellation safe point, quota, schema, recovery, durability, boundary encoding, lowering, machine representation, or family behavior is published with them, and no performance, storage layout, or physical-representation claim is made.

## Finite float algorithms (`GNT-40.5`)

`GNT-40.5-finite-float-algorithms` admits exactly two unary finite-float algorithms — `negate` and `abs` — and exactly two binary finite-float algorithms — `minimum` and `maximum` — over the canonical `Float` domain, whose finite binary64 values normalize either signed zero to the canonical zero. Each algorithm is pure and exact and all four are total: `negate` publishes the sign-flipped operand — the canonical zero at the canonical zero — `abs` its magnitude, and `minimum` and `maximum` the lesser and the greater of their operands under the canonical `Float` value's own total order, so an equal pair publishes that equal value; none refuses an operand, rounds, substitutes a host math library or ambient rounding mode, or publishes a failure code. `UnaryFloatAlgorithm::apply` and `BinaryFloatAlgorithm::apply` publish these algorithms in the model. No float arithmetic beyond the published negation, magnitude, and order selection, and no fused operation, remainder, conversion, transcendental or bounded intrinsic, parsing, formatting, rounding-mode control, exception flag, work limit, cancellation safe point, quota, schema, recovery, durability, boundary encoding, lowering, machine representation, or family behavior is published with them, and no performance, storage layout, or physical-representation claim is made.

## Canonical numeric text (`GNT-40.6`)

`GNT-40.6-canonical-numeric-text` publishes canonical text for the canonical `Int` and finite `Float` domains, adopting `GNT-35.2-literal-formation-and-canonical-text`'s rule: an `Int`'s text is an optional `-` with decimal digits, no leading zero on a multi-digit magnitude and no negative zero; a `Float`'s text is the canonical number spelling of its normalized binary64 value, the same spelling `GNT-8.2` uses for canonical JSON numbers. Formatting is total and publishes exactly the canonical text; parsing admits exactly the canonical text of one value and publishes nothing otherwise, so a non-canonical spelling is refused rather than normalized and `-0` is refused for both domains; the two directions round-trip exactly. `format_canonical_int`, `parse_canonical_int`, `format_canonical_float`, and `parse_canonical_float` publish this surface in the model. No source-literal grammar, digit separator, radix prefix, type suffix, bare `+`, trailing decimal point, non-finite or `NaN` text, rounding, host text facility, locale formatting, other type's text, work limit, cancellation safe point, quota, schema, recovery, durability, boundary encoding, lowering, machine representation, or family behavior is published with them, and no performance, storage layout, or physical-representation claim is made.

## Canonical integer overflow modes (`GNT-40.7`)

`GNT-40.7-canonical-integer-overflow-modes` publishes `GNT-35.3-checked-arithmetic-and-overflow-modes`'s explicit modes — `refuse`, `wrapping`, and `saturating` — for the five binary checked integer algorithms of `GNT-40.2-checked-integer-algorithms` over the canonical `Int` domain of `GNT-5.1`. `refuse` publishes the exact result or refuses under `integer-overflow`; `saturating` publishes the exact result or the domain's nearer bound; `wrapping` publishes the canonical value whose distance above the domain minimum is the exact result's distance above that minimum reduced modulo the domain's span of `2^54 - 1` values; both total modes use the canonical range rather than the declared-width behaviour `GNT-35.3-checked-arithmetic-and-overflow-modes` gives a fixed-width scalar, and no mode applies implicitly. A zero divisor refuses in every mode, and the unary negation needs no mode because the domain is symmetric. `apply_in_mode` publishes these modes in the model. No mode for the negation, conversions, bit operations, or finite-float algorithms, no implicit overflow behavior, unchecked arithmetic, narrower or wider operand, coercion, work limit, cancellation safe point, quota, schema, recovery, durability, boundary encoding, lowering, machine representation, or family behavior is published with them, and no performance, storage layout, or physical-representation claim is made.
